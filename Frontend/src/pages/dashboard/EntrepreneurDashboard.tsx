...
export const EntrepreneurDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      if (!user) return;
      const [requests, investors, meetings, dashboard] = await Promise.all([
      ...
      const now = new Date();
      setUpcomingMeetings(
        (meetings || []).filter((meeting) => new Date(meeting.startTime) > now && meeting.status !== 'rejected').length
      );
      setProfileViews(dashboard?.insights?.recentActivity?.length || 0);
    };
    load();
  }, [user]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            {(user as any).startupName
              ? `Managing ${(user as any).startupName} — here's what's happening today`
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Link to="/meetings">
        <Card className="bg-accent-50 border border-accent-100 hover:border-accent-200 transition-colors">
          <CardBody>
            <div className="flex items-center">
          ...
          <CardBody>
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-xl font-semibold text-success-900">{profileViews}</h3>
              </div>
            </div>
          </CardBody>
        </Card>
