...
export const ChatUserList: React.FC<ChatUserListProps> = ({ conversations }) => {
  ...
  const [participantUsers, setParticipantUsers] = useState<Record<string, any>>({});
  ...
  useEffect(() => {
    const loadParticipants = async () => {
      ...
      const userMap = Object.fromEntries(uniqueIds.map((id, index) => [id, users[index]]));
        ...
        <div className="space-y-1">
          {conversations.length > 0 ? (
            conversations.map(conversation => {
              ...
              const otherUser = participantUsers[otherParticipantId];
