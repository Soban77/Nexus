...
export const InvestorDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      if (!user) return;
      const [ents, reqs, dashboard] = await Promise.all([
      ...
      setRequests(reqs || []);
      setActiveDeals(dashboard?.insights?.activeDeals || 0);
    };
    load();
  }, [user]);
  ...
  const requestedEntrepreneurIds = sentRequests.map((req: any) => String(req.entrepreneur._id || req.entrepreneurId || req.entrepreneur));
