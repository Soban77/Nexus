...
export const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      setMessages(msgs || []);
      const partner = await findUserById(userId);
  ...
  const startCall = (withVideo: boolean) => {
    ...
    if (!roomId) return;
    navigate(`/call/${roomId}${withVideo ? '' : '?audio=1'}`);
  };
            ...
            <div className="border-b border-gray-200 p-4 flex justify-between items-center">
              ...
              <div className="flex space-x-2">
                ...
                <Button
                  ...
                  className="rounded-full p-2"
                  aria-label="Info"
                  onClick={() => navigate(chatPartner.role === 'investor' ? `/profile/investor/${chatPartner.id}` : `/profile/entrepreneur/${chatPartner.id}`)}
                >
                  <Info size={18} />
                </Button>
              </div>
              ...
              {messages.length > 0 ? (
                <div className="space-y-4">
                  {messages.map(message => {
                    const sender = message.senderId === currentUser.id ? currentUser : chatPartner;
                    return (
                      <ChatMessage
                        key={message.id}
                        message={message}
